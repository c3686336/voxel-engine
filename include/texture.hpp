...
public:
    ...
    template <typename T>
    Texture2D(
        std::span<T> image, gl::GLsizei levels, gl::GLenum internal_format,
        gl::GLenum format, gl::GLsizei width, gl::GLsizei height,
        bool generate_mips
    ) {
        using namespace gl;
        ...
        glTextureSubImage2D(
            texture, 0, 0, 0, width, height, format, GL_UNSIGNED_BYTE,
            image.data()
        );
        if (generate_mips) {
